...
import rnd.poc.oauth2.client.OAuth2ResourceServerProperties;
import rnd.poc.oauth2.client.TransactionClient;
...
import rnd.poc.oauth2.client.OAuth2WebClient;
...
@SpringBootTest(classes = OAUTH2ClientAppMain.class)
@ActiveProfiles("it")
public class TransactionClientIT {
    ...
    @Autowired
    @Qualifier(OAuth2WebClient.TRX_OAUTH2_WEB_CLIENT)
    ...
    void retrievingStatusesForMultipleTransactions() throws InterruptedException {
        ...
        Assertions.assertEquals(TransactionClient.TransactionDTO.TransactionStatus.SUCCESSFUL, requireNonNull(firstTransactionResponse).status());
        ...
        Assertions.assertEquals(TransactionClient.TransactionDTO.TransactionStatus.FAILED, requireNonNull(secondTransactionResponse).status());
    }
